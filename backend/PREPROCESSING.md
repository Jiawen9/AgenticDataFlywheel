...
批次列表包含已有01／02产物的批次，因此现有 validation 数据仍可查看和建树，不补造生成／采集记录。没有采集完成登记的旧批次不可通过预处理按钮重新扫描。自备原始轨迹可使用“导入已有 Rollout”登记成新批次，再由预处理或 Pipeline 执行转换和标框；CLI仍可用于明确指定的自备轨迹。详见 [已有 Rollout 导入](ROLLOUT_IMPORTS.md)。
