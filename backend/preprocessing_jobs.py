...
class PreprocessingJobManager:
    ...
    def _reuse_success(self, job):
        ...
        for artifact in job["artifacts"]:
            self.artifacts.resolve_file(artifact, "result.json")
    ...
    def _run(self, job_id):
        ...
        try:
            ...
            if conversion is None:
                ...
                conversion = self.artifacts.publish(job["batch_id"], "01_conversion", payload,
                    workbooks={output.name: output}, source_refs=source_refs,
                    metadata={**counts, "raw_root": snapshot["raw_root"], "preprocessing_job_id": job_id,
                              "input_snapshot_ref": {"path": job["input_path"], "sha256": job["input_sha256"]}})
                saved = [conversion]
                self._update(job_id, artifacts=saved)
            json_path = self.artifacts.resolve_file(conversion, "result.json")
            payload = json.loads(json_path.read_text(encoding="utf-8"))
            if annotation is None:
                self._update(job_id, stage="annotating", completed_steps=0,
                             total_steps=len(payload["sheets"][SHEET]))
                ...
                from .trajectory_context import annotation_batch_lock
                with annotation_batch_lock(job["batch_id"], self.root):
                    current = sorted(self.artifacts.list(job["batch_id"], "02_annotation"),
                                     key=lambda item: item["version"])
                    current_version = current[-1]["version"] if current else None
                    if current_version != job.get("base_annotation_version"):
                        raise PreprocessingError("预处理期间标框版本已变化，已保留当前人工结果；请刷新后重新开始预处理")
                    annotation = self.artifacts.publish(job["batch_id"], "02_annotation", result,
                        workbooks={output.name: output}, source_refs=[conversion],
                        metadata={**counts, "raw_root": snapshot["raw_root"], "model": job["config"]["model"],
                                  "preprocessing_job_id": job_id})
                saved = [conversion, annotation]
            else:
                self.artifacts.resolve_file(annotation, "result.json")
        ...
        with self._lock:
            ...
            for batch_id in batch_ids:
                ...
                if "00_collection" in latest:
                    try:
                        stored = json.loads(self.artifacts.resolve_file(latest["00_collection"],
                ...
                if preview:
                    try:
                        payload = json.loads(self.artifacts.resolve_file(preview, "result.json").read_text(encoding="utf-8"))
