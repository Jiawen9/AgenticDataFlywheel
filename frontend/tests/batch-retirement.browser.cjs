...
async function main() {
  ...
  try {
    ...
    await context.route('**/*', async route => {
      ...
      if (name === '/api/phone-factory/remote/status') return json({ ok: true, statuses: [] })
      if (name === '/api/phone-factory/remote/adb-devices') return json({ ok: true, devices: [] })
      if (name === '/api/phone-factory/collection-runs') return json({ runs: [] })
      if (name === '/api/phone-factory/config') return json({ sampling_enabled: false, temperature: 0.7, top_p: 0.85, use_experience_lib: false })
      if (name === '/api/task-generation/collection-batches' || name === '/api/phone-factory/batches') return json({ batches: ['batch-a', 'batch-b'].filter(id => !published.has(id)).map(id => ({ batch_id: id, kind: 'task_generation', created_at: '2026-09-20', task_count: 1, apps: ['App'], filename: id + '.xlsx' })) })
      if (name.startsWith('/api/task-generation/collection-batches/') || name.startsWith('/api/phone-factory/batches/')) return json({ batch_id: name.split('/')[4], kind: 'task_generation', source_job_id: 'job-a', created_at: '2026-09-20', task_count: 1, apps: ['App'], filename: 'batch-a.xlsx', snapshot: { tasks: [] } })
