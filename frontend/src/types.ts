...
export interface TaskGenerationSourceFile {
  name: string
  exists: boolean
  relative_path: string
  size_bytes: number
...
export interface TaskGenerationSource {
  ready: boolean
  location: string
  files: TaskGenerationSourceFile[]
  errors: string[]
  filters: {
    apps: string[]
    scenes: string[]
...
export type TaskGenerationJobStatus = 'queued' | 'running' | 'succeeded' | 'failed' | 'interrupted'

export interface TaskGenerationJob {
  job_id: string
  kind: 'knowledge' | 'scene_match' | 'variant'
