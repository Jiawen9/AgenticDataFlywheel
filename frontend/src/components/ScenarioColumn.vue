...
.scenario-column{background:var(--studio-surface,#fff)}
.column-item:hover{border-color:var(--line);background:var(--studio-surface,#fff)}
.column-item.selected{border-color:var(--accent);background:var(--studio-accent-soft,#f0fdfa)}
.column-item--app{border-color:var(--line);background:var(--studio-surface-soft,#f8fafc)}
.inline-creator{border-color:var(--accent)}
.column-header h3,.item-label,.column-empty strong{color:var(--studio-ink,var(--ink))}
.column-kicker,.item-level,.item-arrow,.item-meta,.item-more,.creator-cancel,.column-empty p{color:var(--studio-muted,var(--muted))}
.column-footer{border-top-color:var(--line)}
</style>
